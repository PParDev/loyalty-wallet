...
const navItems = [
  ...
  { href: "/dashboard/locations", label: "Sucursales", icon: IconMap },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/businesses/me")
      ...
      .then((res) => {
        ...
        if (res.success && res.data?.onboardingCompleted === false && pathname !== "/dashboard/onboarding") {
  ...
  const isKiosk = pathname === "/dashboard/kiosk";

  // En modo kiosk, renderizar sin sidebar ni nav
  if (isKiosk) {
    return <>{children}</>;
  }
        ...
        <div className="px-3 py-4 border-t border-gray-100">
          ...
          {session?.user.role === "admin" && (
            <Link
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-sm font-medium text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors mt-1"
            >
              <IconKiosk />
              Modo Kiosk
            </Link>
